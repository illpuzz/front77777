<!-- src/components/reviews/modals/ReviewEditModal.vue - 編輯評價的模態窗口組件 (修改以支持不同的營地ID) -->
<template>
  ...
  export default {
    ...
    setup(props, { emit }) {
      ...
      watch(() => props.review, async (newReview) => {
        ...
        try {
          // 從後端獲取評價的圖片列表
          const response = await axiosapi.get(`/api/review-images/review/${newReview.id}`);
      ...
      onMounted(() => {
        console.log('=========評價編輯模態窗==========');
        console.log('模態窗已載入');
        console.log('營地ID:', props.campSiteId);
        console.log('用戶角色:', props.review ? '可編輯評價' : '等待數據');
        console.log('===================================');
      ...
      const saveReviewEdit = async () => {
        ...
        try {
          ...
          if (imagesToDelete.value.length > 0) {
            ...
            for (const imageId of imagesToDelete.value) {
              try {
                ...
                const response = await axiosapi.delete(`/api/review-images/${imageId}`);
          ...
          const updateData = {
            ...editData.value,
            id: props.review.id,
            campSiteId: props.campSiteId, // 使用props的campSiteId
            userId: props.review.userId
          };
          
          const response = await axiosapi.put(`/api/reviews/${props.review.id}`, updateData);
